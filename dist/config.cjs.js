'use strict';Object.defineProperty(exports,"__esModule",{value:!0});function _interopDefault(a){return a&&"object"==typeof a&&"default"in a?a["default"]:a}var fs$1=_interopDefault(require("fs-extra")),os=_interopDefault(require("os")),path=_interopDefault(require("path")),browsersync=_interopDefault(require("rollup-plugin-browsersync")),postcss=_interopDefault(require("rollup-plugin-postcss")),replace=_interopDefault(require("rollup-plugin-replace")),resolve=_interopDefault(require("rollup-plugin-node-resolve")),sass=_interopDefault(require("@csstools/postcss-sass")),mixins=_interopDefault(require("postcss-mixins")),pxtorem=_interopDefault(require("postcss-pxtorem")),easyImport=_interopDefault(require("postcss-easy-import")),hexrgba=_interopDefault(require("postcss-hexrgba")),presetEnv=_interopDefault(require("postcss-preset-env")),favicons=_interopDefault(require("favicons")),fs$2=_interopDefault(require("fs")),babel$1=_interopDefault(require("rollup-plugin-babel")),commonjs=_interopDefault(require("rollup-plugin-commonjs")),json=_interopDefault(require("rollup-plugin-json")),minify=_interopDefault(require("rollup-plugin-babel-minify"));const babel=({library:a,plugins:b,presets:c,root:d,targets:e})=>{const f=void 0===a?"vanilla":a.toLowerCase(),g=b,h=c,i=d,j=void 0===e?{node:"8",browsers:"> 5% in US"}:e,k={presets:[["@babel/preset-env",{targets:j}]],plugins:["@babel/plugin-proposal-object-rest-spread","@babel/plugin-syntax-dynamic-import","transform-postcss",["module-resolver",{root:["./"],alias:{"@src":`${i}src`,"@containers":`${i}src/containers`,"@components":`${i}src/components`,"@css":`${i}src/css/`,"@styles":`${i}src/css/`,"@config":`${i}config`,"@dist":`${i}.dist`,"@lib":`${i}lib`,"@public":`${i}public`,"@render":`${i}src/render`,"@test":`${i}test`}}]]};return"react"===f?k.presets.push("@babel/preset-react"):k.plugins.push(["@babel/plugin-transform-react-jsx",{pragma:"dom",// default pragma is React.createElement
...
}},e=`${a}/icons/`;favicons(`${b}public/icon.png`,d,(b,c)=>b?void console.log(b.message):void(!fs$2.existsSync(a)&&fs$2.mkdirSync(a),fs$2.unlink(e,()=>{fs$2.existsSync(e)||fs$2.mkdirSync(e),c.images.map(a=>{a.name&&fs$2.writeFile(`${e}${a.name}`,a.contents,b=>!b||(console.log("COULD NOT SAVE",`${e}${a.name}`,b),!1))}),c.files.map(a=>{a.name&&fs$2.writeFile(`${e}${a.name}`,a.contents,b=>!b||(console.log("COULD NOT SAVE",`${e}${a.name}`,b),!1))}),c.html.map(a=>{a.name&&fs$2.writeFile(`${e}${a.name}`,a.contents,b=>!b||(console.log("COULD NOT SAVE",`${e}${a.name}`,b),!1))})})))},fs=require("fs"),ENABLE_SW="production"===process.env.NODE_ENV||process.env.ENABLE_SW,generateSW=({ROOT:a})=>{const b=require(`${a}package.json`);// eslint-disable-line
...
name:"diskCleaner",generateBundle(){fs$1.existsSync(b)&&(e&&(console.log("CLEAN",e),console.log(`Frosti: cleaning path: ${b}`.white),fs$1.removeSync(b)),favicon({DIR_OUTPUT:b,ROOT:c,CONFIG:a}),generateSW({ROOT:c}))}},resolve({browser:!0}),replace({"process.env.NODE_ENV":JSON.stringify(f)}),postcss({config:!1,extract:!0,loaders:["sass","stylus","less"],plugins:[sass(),mixins,pxtorem,easyImport,hexrgba,presetEnv({autoprefixer:{flexbox:"no-2009"},stage:3,features:{"nesting-rules":!0}})]}),...i];return process.env.DEV_SERVER&&j.push(browsersync({open:!1,files:["src/**/*","config/**/*"],https:{key:path.join(g,"localhost-key.pem"),cert:path.join(g,"localhost.pem")},// server: DIR_OUTPUT
proxy:`https://localhost:${h}`})),j},rollup=({// REQUIRED
config:a,root:b,// OPTIONAL
library:c,input:d,output:e,context:f,plugins:g})=>{const h=d,i=void 0===c?"vanilla":c.toLowerCase(),j=e,k=f,l=g,m=b,n=`${m}/.dist/`;void 0===h&&(fs$1.pathExistsSync(`${m}/src/Client.js`)?d=`${m}/src/Client.js`:fs$1.pathExistsSync(`${m}/src/Client.jsx`)?d=`${m}/src/Client.jsx`:fs$1.pathExistsSync(`${m}/src/render/Client.js`)?d=`${m}/src/render/Client.js`:fs$1.pathExistsSync(`${m}/src/render/Client.jsx`)&&(d=`${m}/src/render/Client.jsx`));const o={input:d,output:{file:`${n}app.js`,format:"iife",sourcemap:!0,globals:{react:"React","react-dom":"ReactDOM"}},plugins:clientPlugins({CONFIG:a,DIR_OUTPUT:n,ROOT:m,LIBRARY:i,CLEAN:!1}),// We load React via cdn in HTML template to reduce bundle size and leverage browser caching
external:["react","react-dom"]};return h&&(o.input=h),j&&(o.output=j),k&&(o.context=k),l&&o.plugins.concat(l),o},rollup$1=({// REQUIRED
config:a,root:b,// OPTIONAL
library:c,input:d,output:e,context:f,plugins:g})=>{const h=d,i=void 0===c?"vanilla":c.toLowerCase(),j=e,k=f,l=g,m=b,n=`${m}/.dist/`;void 0===d&&(fs$1.pathExistsSync(`${m}/src/Server.js`)?d=`${m}/src/Server.js`:fs$1.pathExistsSync(`${m}/src/Server.jsx`)?d=`${m}/src/Server.jsx`:fs$1.pathExistsSync(`${m}/src/Express.js`)?d=`${m}/src/Express.js`:fs$1.pathExistsSync(`${m}/src/Express.jsx`)&&(d=`${m}/src/Express.jsx`));const o={input:d,output:{file:`${n}ssr.js`,format:"cjs",globals:{colors:"colors"}},context:"window",external:["chokidar","colors","compression","cors","express","spdy","fs","os","path","redirect-https","react-helmet"],plugins:clientPlugins({CONFIG:a,DIR_OUTPUT:n,ROOT:m,LIBRARY:i,CLEAN:!1})};return h&&(o.input=h),j&&(o.output=j),k&&(o.context=k),l&&o.plugins.concat(l),o};/* eslint-disable max-len, no-console */var index={client:rollup,server:rollup$1};exports.babel=babel,exports.rollup=index;
