...
const generateSW = ({ ROOT }) => {
  const pkg = require(`${ROOT}package.json`); // eslint-disable-line

  if (ENABLE_SW) {
    const src = `${ROOT}public/service-worker.js`;
    const dest = `${ROOT}.dist/service-worker.js`;
